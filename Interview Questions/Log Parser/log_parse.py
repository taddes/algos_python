...
def parse_log(log):
    log = log.strip()
    date_portion = log.split('[')[0].strip()
    device_stat = log[-3:].strip()
    print(date_portion)
    print(device_stat)
    date = datetime.strptime(date_portion.strip(), '%b %d %H:%M:%S:%f')
    # print(date)
    
parse_log('Jul 11 16:11:51:490 [139681125603136] dut: Device State: ON')
